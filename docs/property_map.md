...
| `roll` (24) | quat | `engine_object.rot_quat` (roll) | **DONE** | 2.8 roll, radians about the forward/nose axis. a2x composes a roll onto the rotation quaternion about the object's LOCAL forward (Z) axis (`a2x/coords.py` set_roll), preserving facing; the X/Z mirror flips the sense so the applied roll is `-r` (pi is sign-invariant -- all the corpus uses: 180-degree warpgate flips). VERIFIED in-engine (A2xTestRange `test_convert_roll` 2/2: facing unchanged + pi flips the right vector). Wired via `_PROP` `("roll", ...)` + tool `_AUTO_PROPS` |
| `pitch` (13) | quat | `engine_object.rot_quat` (pitch) | **DONE** | 2.8 pitch (nose up/down), about the right axis. a2x composes a pitch onto the quaternion about the object's LOCAL right (X) axis (`a2x/coords.py` set_pitch), preserving facing. Documented as radians, but the corpus uses magnitudes > pi (10/20/90/120) -- the parser is degrees everywhere else, so magnitudes > pi are treated as degrees. VERIFIED in-engine (A2xTestRange `test_convert_pitch` 2/2: 90deg -> nose vertical, 0.5rad -> small tilt). Wired via `_PROP` `("pitch", ...)` + tool `_AUTO_PROPS` |
